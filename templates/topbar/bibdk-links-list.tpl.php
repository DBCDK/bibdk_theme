 ...
 *
 * @parameters:
 * - $attributes: array with attributes that should be attached the outer list element (<ul>).
 * - $items: array with items. Each item will represent a link i the list.
 *  - $item: a link that will be wrapped in a <li>.
 * $label: if a label is provided it will be printed wrapped in a <li> at top of the list.
 *
 * @see _bibdk_theme_get_offcanvas_menu_list() in template.php for example of usage.
 */
?>

<ul<?php print drupal_attributes($attributes);?>>
  <?php if($label): ?>
    <li><label><?php print $label; ?></label></li>
  <?php endif; ?>
